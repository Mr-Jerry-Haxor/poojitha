  ...
  const fetchData = async () => {
    try {
      ...
      const [contentsRes, chartsRes] = await Promise.all([
        axios.get(`http://localhost:3000/admin/contents/${pageType}`, {
          headers: { Authorization: `Bearer ${token}` }
        }),
        axios.get(`http://localhost:3000/admin/charts/${pageType}`, {
          headers: { Authorization: `Bearer ${token}` }
        })
      ]);
  ...
  const handleSubmitContent = async (e) => {
    ...
    try {
      const endpoint = editMode === 'content' ? 
        ...
        'http://localhost:3000/admin/add_content';
  ...
  const handleSubmitChart = async (e) => {
    ...
    try {
      const endpoint = editMode === 'chart' ? 
        `http://localhost:3000/admin/chart/${editId}` : 
        'http://localhost:3000/admin/add_chart';
